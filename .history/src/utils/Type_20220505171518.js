global["Type"] = class Type {
  ...
  setter(value, options) {
    ...
    if(value === null) {
      ...
      throw new Error(this.#setterError(-5, {datatypes, value}))
    }else {
  ...
  #setterError(status, { name, minlength, maxlength, datatypes, value }={}) {
    switch (status) {
      ...
      case -5: return `value has not right data type OR it is required. Expected one of=>${datatypes.join(",")}, but recieved null.``
