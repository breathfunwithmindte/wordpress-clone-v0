global["Type"] = class Type {
  ...
  setter(value, options) {
    ...
    if(value === null) {
      ...
      if(datatype_is_included && !isNaN(Number(false))) return Number(value);
      return value;
    }
  ...
  #setterError(status, { name, minlength, maxlength, datatypes, value }={}) {
    switch (status) {
      ...
      case -4: return `${name}::` + `value has not right data type. Expected one of=>${datatypes.join(",")}, but recieved ${typeof value}`;
      case -5: return `${name}::` + `value has not right data type OR it is required. Recieved null.`
