global["Type"] = class Type {
  ...
  setter(value, options) {
    ...
    if(!datatypes.some(d => d === typeof value)) throw new Error(this.#setterError(-4, {datatypes}))
  ...
  #setterError(status, { name, minlength, maxlength, datatypes }={}) {
    switch (status) {
      ...
      case -4: return `value has not right data type. Expected one of=>${datatypes.join(",")}, but recieved`;
