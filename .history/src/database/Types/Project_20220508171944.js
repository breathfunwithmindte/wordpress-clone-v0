...
const { products_standard, orders_standard } = require("../DefaultFields");

module.exports = class Project extends Type {
  /**
   * @param  {string} name
   * @param  {list<import("./Types/Field")>} product_schema
   * @param  {list<import("./Types/Field")>} costumer_schema
   * @param  {list<import("./Types/Field")>} order_schema
   * @param  {list<import("./Types/Field")>} review_schema
   * @param  {list<import("./Types/Field")>} blog_schema
   * @param  {list<import("./Types/Field")>} comment_schema
   * @param  {list<import("./Types/Field")>} category_schema
   * @param  {list<import("./Types/Field")>} cart_schema
   * @param  {list<import("./Types/Field")>} product_quantity_schema
  ...
  constructor(name,
    ...
    super();
    this.product_schema = [
      ...product_schema, ...products_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
    ...
    this.product_quantity_schema = product_quantity_schema.map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
  }
  ...
  #getArrayFields (modelname) {
    ...
    if(modelname === "products-quantities") return this.product_quantity_schema;
    return []
  }
