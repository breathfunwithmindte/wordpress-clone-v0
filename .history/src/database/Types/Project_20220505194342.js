    ...
    try{
      ...
      this.#getArrayFields(modelname).map(c => {
        if(c.array) {
          ...
          delete schema[c.name][0]["name"];
          if(schema[c.field][0]["field"] !== "id") { delete schema[c.field][0]["ref"] }
        }else {
          schema[c.name] = {...c, type: this.#getSchemaType(c.field)}
          delete schema[c.name]["name"];
          if(schema[c.field]["field"] !== "id") { delete schema[c.field]["ref"] }
        }
      })
