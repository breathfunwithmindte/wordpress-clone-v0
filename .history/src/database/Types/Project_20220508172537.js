...
module.exports = class Project extends Type {
   ...
   * @param  {list<import("./Field")>} product_quantity_schema
  ...
  constructor(name,
    ...
    this.costumer_schema = [
      ...costumer_schema, ...costumers_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
    this.order_schema = [
    ...
    this.review_schema = [
      ...review_schema, ...reviews_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
    this.blog_schema = [
      ...blog_schema, ...blogs_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
    this.comment_schema = [
      ...comment_schema, ...comments_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
    this.category_schema = [
      ...category_schema, ...categories_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
    this.cart_schema = [
      ...produccart_schemat_schema, ...carts_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
    this.product_quantity_schema = [
      ...product_quantity_schema, ...product_quantities_standard
    ].map(
      (p) => new Field(p._id.toString(), p.field, p.name, p.default_value, p.minlength, p.maxlength, p.required, p.lowercase, p.ref, p.array, p.enum_arr)
    );
  }
